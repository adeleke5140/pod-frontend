...
export const Repos = () => {
  ...
  function copyLink(hash: string) {
    ...
    const url = repo && `${mintDomain}/mint?pHash=${repo.projectHash}`;
    if (url) {
      void copy(url);
      setCopiedRepo(hash);
      ...
      <div className="text-left">
        ...
        <ul className="mb-4 flex gap-4">
          <button
            ...
            className={`cursor-pointer rounded-lg bg-blue-50 px-2 py-1 font-semibold transition-all hover:bg-blue-100 ${
              repoDisplay === "all" ? "bg-blue-100 font-bold text-blue-600" : ""
            }`}
          >
            All repos
          </button>
          ...
          {repoDisplay === "all"
            ? repos.map((repo) => (
                <Link
                  ...
                  key={repo.name}
                  className={`${
                    repo.isAllowedMint ? "pointer-events-none" : ""
